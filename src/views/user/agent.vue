  ...
  methods: {
    ...
    async getList(meta, data) {
      ...
      const resData = await users_agent(meta || this.meta, data).then(res => {
        ...
        if (res.code === 0) {
          ...
          resData = this.formatConversion([], resData)
          console.log('agent data', resData)
          if (data) {
            return resData
          }
          this.list = resData
    ...
    *********************************/
    toggleFoldingClass(params) {
      return params.childNum > 0 ? (this.foldList.indexOf(params.__identity) === -1 ? 'el-icon-arrow-right' : 'el-icon-arrow-down') : ''
    },
    /** *******************************
      ** Fn: formatConversion
      ** Intro: 将树形接口数据扁平化
